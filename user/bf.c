...
void run(char *code, int options);
...
int main(int argc, char *argv[])
{
	...
	if (HAS_OPTION(OPT_HELP)) {
		...
		if(0>fstat(fp, &fp_stat))
			printf(stderr, "Something wrong with fstat\n");
		...
		printf(stderr, "bf: no input files\n" DBFI_USAGE);
		exit();
	}
		...
		switch (commands[i].ctype) {
			...
			case BF_DEBUG:
				if (HAS_OPTION(OPT_DEBUG)) {
					printf(stdout, "{%d=>%d}", pointer, tape[pointer]);
				}
		}
	}
